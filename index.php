        ...
        <div class="nav projects-filter text-light">
          ...
          <?php
            ...
            foreach ( $categories as $category ) {
              ?>
              <a class="nav-link" href="#<?php echo $category->slug; ?>" data-filter=".f<?php echo $category->slug; ?>" title="Filter <?php echo $category->name; ?>"><?php echo $catogory->name; ?></a>
              <?php
            }
          ?>
        ...
        <?php if ( has_post_thumbnail() ) : ?>
          <?php
            ...
            foreach ($currentCategories as $currentCategory) {
              ...
              $categories = array_push( $categories, $currentCategory -> slug );
            }
            $meta = join('<span>/</span>', array_map(function($a) { return '<span class="project-category-item">' . $a . '</span>';}, $categories));
          ?>
          <figure class="col-lg-4 fweb<?php echo $slugs; ?>">
            <a class="card bg-dark text-white shadow text-center" href="<?php the_permalink(); ?>">
              <?php the_post_thumbnail( 'portfolio-thumbnail', array( 'class' => 'img-fluid card-img' ) ); ?>
